...
@pytest.mark.asyncio
async def test_chat_completions_with_system(aibrary: AiBrary):
...
@pytest.mark.asyncio
async def test_audio_transcriptions(aibrary: AiBrary):
...
@pytest.mark.asyncio
async def test_automatic_translation(aibrary: AiBrary):
    async def _inner_fun(model: Model):
        last_exception = None
        for attempt in range(MAX_RETRIES):
            try:
...
@pytest.mark.asyncio
async def test_audio_speech_creation(aibrary: AiBrary):
    async def _inner_fun(model: Model):
        last_exception = None
        for attempt in range(MAX_RETRIES):
            try:
...
@pytest.mark.asyncio
async def test_image_generation_with_multiple_models(aibrary: AiBrary):
    async def _inner_fun(model: Model):
        last_exception = None
        for attempt in range(MAX_RETRIES):
            try:
